  ...
  if (step === "loading" || !session) {
    return (
      <BusinessShell back={{ to: "/path/business/home", label: "ბიზნეს ინგლისური" }}>
        <BizCard>
          <p className="ka text-[#5B6473]">გასაუბრება მზადდება შენი პროფილისთვის...</p>
  ...
  return (
    <BusinessShell back={{ to: "/path/business/home", label: "ბიზნეს ინგლისური" }}>
      ...
      {step === "briefing" && (
        <div className="space-y-3">
          ...
          <BizCard className="border-l-4 border-l-[#C9A227]">
            ...
            <h2 className="ka text-xl font-bold text-[#1E2A44] mt-2">{b.roleTitleKa}</h2>
            <p className="text-sm text-[#5B6473] mt-0.5">{b.roleTitle}</p>
            ...
            <div className="mt-4 p-3 rounded-lg bg-[#FAF7F0] border border-[#E7E2D5]">
              <p className="ka text-xs font-semibold text-[#1E2A44]">კომპანიის შესახებ</p>
              <p className="ka text-sm text-[#374151] mt-1">{b.aboutCompanyKa}</p>
            </div>
      ...
      {(step === "interview" || step === "verdict") && (
        <div className="space-y-3">
          {/* Interview "set" card */}
          <div className="rounded-2xl bg-gradient-to-br from-[#1E2A44] via-[#1A2540] to-[#15203A] text-[#F7F1E3] p-4 shadow-[0_12px_30px_-15px_rgba(30,42,68,0.5)]">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#C9A227] text-[#1E2A44] grid place-items-center font-bold text-sm">
                {initials(b.interviewerName)}
              </div>
              <div className="min-w-0">
                <p className="text-sm font-semibold truncate">{b.interviewerName}</p>
                <p className="ka text-[11px] text-[#F7F1E3]/70 truncate">
                  {b.interviewerTitle} · {b.companyName}
                </p>
              </div>
              <div className="ml-auto ka text-[10px] uppercase tracking-wider text-[#F2D680]">
          ...
          <div
            ref={chatRef}
            className="bg-white border border-[#E7E2D5] rounded-2xl p-3 max-h-[420px] overflow-y-auto space-y-2"
          >
            {history.map((t, i) => (
              <div
                key={i}
                className={`max-w-[85%] p-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${
                  t.role === "interviewer"
                    ? "bg-[#FAF7F0] border border-[#E7E2D5] text-[#1E2A44] mr-auto rounded-bl-sm"
                    : "bg-[#1E2A44] text-[#F7F1E3] ml-auto rounded-br-sm"
                }`}
              >
                {t.text}
              </div>
            ...
            {thinking && (
              <div className="max-w-[85%] p-3 rounded-2xl bg-[#FAF7F0] border border-[#E7E2D5] mr-auto inline-flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-[#1E2A44] animate-[blink_1s_infinite]" />
                <span className="w-1.5 h-1.5 rounded-full bg-[#1E2A44] animate-[blink_1s_.15s_infinite]" />
                <span className="w-1.5 h-1.5 rounded-full bg-[#1E2A44] animate-[blink_1s_.3s_infinite]" />
              </div>
            )}
          </div>
          ...
          {step === "interview" && (
            <BizCard>
              ...
              <textarea
                ...
                className="mt-2 w-full min-h-[110px] p-3 rounded-lg border border-[#E7E2D5] text-sm text-[#1E2A44] outline-none focus:border-[#1E2A44] resize-y"
                disabled={thinking}
              />
      ...
      {step === "debrief" && debrief && (
        <div className="space-y-3">
          ...
          {debrief.hurtChances?.length > 0 && (
            <BizCard>
              <p className="ka text-xs font-semibold text-[#B45309]">↗ რამ დააზიანა შენი შანსები</p>
              <div className="mt-2 space-y-2">
          ...
          {debrief.keyPhrases?.length > 0 && (
            <BizCard>
              <p className="ka text-xs font-semibold text-[#1E2A44]">3 ფრაზა შემდეგისთვის</p>
              <div className="mt-2 space-y-2">
                {debrief.keyPhrases.map((p, i) => (
                  <div key={i} className="p-3 rounded-lg bg-[#FAF7F0] border border-[#E7E2D5]">
                    ...
                    <p className="ka text-[11px] text-[#C9A227] mt-1">📍 {p.whenKa}</p>
                  </div>
                ))}
              </div>
            </BizCard>
          ...
          <BizCard className="bg-[#FFFBEA] border-[#F2E6B0]">
            <p className="ka text-xs font-semibold text-[#C9A227]">🎯 ერთი რამ, რაც უნდა ივარჯიშო</p>
        ...
        <div className="relative">
          ...
          <BizCard className="mt-3">
            <div className="flex items-end justify-between">
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-[#1E2A44]">{score >= 0 ? `+${score}` : score}</div>
  ...
  return (
    <div className="mb-4">
      <div className="flex items-center justify-between mb-2">
        <h1 className="ka text-xl font-bold text-[#3d6b9e] inline-flex items-center gap-2"><Briefcase size={18} strokeWidth={2.25} /> გასაუბრება</h1>
        <span className="ka text-[11px] text-[#5B6473]">
          {curriculum
